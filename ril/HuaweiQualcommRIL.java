...
/**
 * Qualcomm RIL for the huawei family.
...
public class HuaweiQualcommRIL extends RIL implements CommandsInterface {
    ...
    private boolean huaweiEmergency = needsOldRilFeature("huaweiEMSReq");

    private Message mPendingGetSimStatus;

    public HuaweiQualcommRIL(Context context, int preferredNetworkType,
            int cdmaSubscription, Integer instanceId) {
        this(context, preferredNetworkType, cdmaSubscription);
    }

    public HuaweiQualcommRIL(Context context, int networkMode,
    ...
    protected Object responseSignalStrength(Parcel p) {
        ...
        }else{ // lte is gsm on huawei/qualcomm cdma stack
            response[7] &= 0xff;
        }
 ...
 | sed -re 's/\{([^,]+),[^,]+,([^}]+).+/case \1: ret = \2(p); break;/'
    ...
    // CDMA FIXES, this fixes  bogus values in nv/sim on huawei cdma family or bogus information from sim card
    private Object
    ...
    // Workaround for Huawei CDMA "ring of death" bug:
    //
    ...
    dial(String address, int clirMode, UUSInfo uusInfo, Message result) {
        if (huaweiEmergency && PhoneNumberUtils.isEmergencyNumber(address)) {
